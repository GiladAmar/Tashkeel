    ...
    for (const backend of ['wasm', 'webgl', 'cpu']) {
        try {
            if (await tf.setBackend(backend)) {
                await tf.ready();
                console.log('Tashkeel: using backend', backend);
                globalThis.__tashkeelBackend = backend; // observability (e2e tests)
                return;
            }
        } catch (e) {
            console.warn(`Tashkeel: backend ${backend} unavailable`, e);
        }
    }
...
async function load_model() {
    ...
    await diacritize(tf, model, 'ا');
    return model;
}

// Lazy + memoized: the service worker wakes for plenty of model-free work
// (Remove tashkeel, opening the paste page, frame probes) and must not pay
// backend init + 10MB of weights + warm-up for those. The add-tashkeel call
...
function getModel() {
    if (!modelPromise) {
        ...
        globalThis.__tashkeelModelReady = attempt.then(() => true, () => false);
    }
    return modelPromise;
}
// observability (e2e tests): force the lazy load and await readiness
globalThis.__tashkeelEnsureModel = () => getModel().then(() => true, () => false);
...
async function invoke(tab, file) {
    ...
    try {
        ...
        const target = frameIds.length > 0
            ...
            // Remove tashkeel restore iframe work)
        ...
        if (file === 'content.js') {
            ...
            await chrome.scripting.executeScript({
                target,
                func: (selectionOnly) => { window.__tashkeelSelectionOnly = selectionOnly; },
                args: [frameIds.length > 0],
            });
        }
        await chrome.scripting.executeScript({target, files: [file]});
    } catch (e) {
        console.warn('Tashkeel: cannot run on this page', e);
    }
}
...
globalThis.__tashkeelInvoke = invoke;
globalThis.__tashkeelInvokeWholePage = (tab) => invokeWholePage(tab);

chrome.runtime.onInstalled.addListener(() => {
    chrome.contextMenus.removeAll(() => {
        // English-first labels: the audience is Arabic learners.
        chrome.contextMenus.create({
            id: 'tashkeel-selection',
            title: 'Add tashkeel (أضف تشكيل)',
            contexts: ['selection'],
        });
        chrome.contextMenus.create({
            id: 'tashkeel-page',
            title: 'Add tashkeel to the whole page',
            contexts: ['page'],
        });
        chrome.contextMenus.create({
            id: 'tashkeel-remove',
            title: 'Remove tashkeel (أزل تشكيل)',
            contexts: ['selection', 'page'],
        });
        chrome.contextMenus.create({
            id: 'tashkeel-paste-page',
...
chrome.contextMenus.onClicked.addListener((info, tab) => {
    if (info.menuItemId === 'tashkeel-remove')
        invoke(tab, 'content_undo.js');
    else if (info.menuItemId === 'tashkeel-page')
        invokeWholePage(tab);
    else if (info.menuItemId === 'tashkeel-paste-page')
...
chrome.commands.onCommand.addListener(async (command, tab) => {
    if (command !== 'add-tashkeel') return;
...
async function handleRequest(port, segments) {
    ...
    try {
        ...
        console.error('Tashkeel: model failed to load', e);
    ...
    try {
        ...
        console.error('Tashkeel: diacritization failed', e);
        post(port, {type: 'fatal', reason: String((e && e.message) || e)});
    }
}

chrome.runtime.onConnect.addListener((port) => {
    if (port.name !== 'tashkeel') return;
