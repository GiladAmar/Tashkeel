...
import {diacritize, diacritize_batch, remove_tashkeel, chunkSegments} from '../text_encoding.mjs';
...
for (let i = 0; i < segments.length; i++) {
    ...
    const clusters = (s) => s.match(/[ء-ي][ً-ْ]*/g) || [];
...
for (let i = 0; i < segments.length; i++) {
    if (remove_tashkeel(packed[i]) !== remove_tashkeel(separate[i]))
