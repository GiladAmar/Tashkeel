...
// `npm run fixtures`. Tests skip cleanly when a fixture is absent, and always
// run against the committed regression-page.html.
...
import {remove_tashkeel, diacritize_batch, chunkSegments} from '../text_encoding.mjs';
    ...
    describe(`real page through the full pipeline: ${fixtureName}`,
        ...
        test('whole-page mode: every segment survives and round-trips exactly', async () => {
            ...
            for (let i = 0; i < segments.length; i++) {
                assert.equal(remove_tashkeel(results[i]), remove_tashkeel(segments[i]),
                    `segment ${i} must round-trip exactly`);
            }

            const dotted = results.filter(r => TASHKEEL_RE.test(r)).length;
            console.log(`# ${fixtureName}: ${segments.length} segments, ${chars} chars, ` +
                `${dotted} received tashkeel, ${(ms / 1000).toFixed(1)}s`);
            assert.ok(dotted / results.length > 0.8,
                `most Arabic segments should receive tashkeel (got ${dotted}/${results.length})`);
        });
