...
import {diacritize, remove_tashkeel} from '../text_encoding.mjs';
...
describe('quantized model', () => {
    ...
    test('diacritization output stays essentially identical', async () => {
        const samples = [
            'هم جزء من جهود الجمعية الوطنية لحماية الطبيعة في البلاد',
            'عنوان رئيسي: شيء ما حدث اليوم صباحا في المدينة الكبيرة.',
            'أقرت الحكومة الميزانية الجديدة للعام الدراسي المقبل.',
            'من المتوقع أن يكون الطقس حارا وجافا في معظم مناطق البلاد.',
            'سجل لاعب كرة القدم ثلاثة أهداف في مباراة الأمس أمام المنافس.',
            'يكشف البحث الجديد عن نتائج مفاجئة حول نمط حياتنا اليومي.',
            'أعلن رئيس البلدية عن خطة بناء جديدة في الأحياء الجنوبية.',
            'قبلت المحكمة الاستئناف وقضت بتعويضات للمدعين في القضية.',
        ];
        let same = 0, total = 0;
        for (const text of samples) {
            const a = await diacritize(tf, f32model, text);
            const b = await diacritize(tf, f16model, text);
            assert.equal(remove_tashkeel(b), text, 'quantized output must round-trip');
