...
function showToast(message) {
    const previous = document.getElementById('__tashkeel-toast');
    if (previous) previous.remove();
    const toast = document.createElement('div');
    toast.id = '__tashkeel-toast';
...
// What this extension has written, so "remove tashkeel" can restore originals
...
function getRegistry() {
    if (!window.__tashkeelOriginals)
        window.__tashkeelOriginals = new WeakMap();
    return window.__tashkeelOriginals;
}
 ...
 *   Remove tashkeel gives back the edited text, not a pre-edit state.
...
function requestDiacritics(segments, pending, {onDone, onFail} = {}) {
    ...
    document.documentElement.setAttribute('data-tashkeel-busy', '');
    ...
    const port = chrome.runtime.connect({name: 'tashkeel'});
    ...
    port.onMessage.addListener((msg) => {
        ...
        if (msg.type === 'result') {
            ...
            console.error('Tashkeel failed:', msg.reason);
            fail('Tashkeel: adding tashkeel failed — the page was restored');
        }
    });
    ...
    port.onDisconnect.addListener(() => {
        if (!finished) fail('Tashkeel: interrupted — the page was restored');
    });
    port.postMessage({type: 'diacritize', segments});
}

// The focused input/textarea. By default it must hold a text selection
// (adding tashkeel targets the selection); pass requireSelection=false for
// operations on the whole field, e.g. Remove tashkeel after the selection
...
function runWholePage() {
    ...
    if (segments.length === 0) {
        // injected into all frames: only the top frame reports, so an
        // Arabic-free iframe doesn't toast over a page that was processed
        if (window === window.top)
            showToast(alreadyDotted > 0
                ? 'Tashkeel: this page already has tashkeel'
                : 'Tashkeel: no Arabic text found on this page');
        return;
    }
